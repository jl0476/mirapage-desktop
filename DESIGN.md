...
- **瀑布流打磨收口**（module3.5.3）：in-flight 尺寸守卫 ✅、archiveOpenError 导航清空 ✅；hasImages 搜索态查明无缺口（守卫源头即全量集合）销账；像素级 scrollTop 锚定补偿保留（avgRatio 全局漂移并入考量，观察守卫效果后另立项）。守卫语义键 watch 收窄了侦听源——本地源 descriptor 重渲染不再重启 browsePosition/关 popover（有意为之的无害偏好）。
