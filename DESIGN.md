...
- **瀑布流打磨收口**（module3.5.3）：in-flight 尺寸守卫 ✅、archiveOpenError 导航清空 ✅；hasImages 搜索态查明无缺口（守卫源头即全量集合）销账；像素级 scrollTop 锚定补偿 ✅ 已交付（2026-09-04 §16.2 G 项，commitMeasuredMap 锚定法：严格锚 + loose 锚（max-bottom/belowOffset）双覆盖、批次状态闭包化、resize↔测量双向让位、目录切换/RO 开始双失效钩 + guardKey/seq 守卫；删孤儿 applyMeasuredBatch；净 +12 用例基线 1306；实机 E2E 探针 v4 复测待跑）。守卫语义键 watch 收窄了侦听源——本地源 descriptor 重渲染不再重启 browsePosition/关 popover（有意为之的无害偏好）。
