...
> 2026-08-15 修订：总像素硬阈值从 4MP 放宽到 8MP（命中率报告
> `docs/superpowers/reports/2026-08-10-masonry-policy-hit-rate.md` 方案 A）。
> 4–8MP 典型手机照不再仅因像素数命中硬闸，落回灰区低优生成；只影响"是否必生成"，
> 输出像素预算维持 §6.4 的 3MP/4MP 不变，已缓存缩略图仍有效。

2–8MP、2–4MB 或尺寸倍率 1.25–1.5 的灰色区间仍生成缩略图，但优先级低于明确的大图。首次没有缓存时继续显示占位，不使用大原图作为临时回退，以免重新引入卡顿。
