...
| 决策点 | 选择 | 理由 |
|---|---|---|
| 生成范围 | 可见区 + 向下 1.5 屏 + 向上 0.5 屏 | 保证正常滚动命中，同时严格限制资源 |
| 生成时机 | 视口驱动，停止滚动后可额外预生成 1 屏 | 不全量生成，不让快速滚动制造大量无效任务 |
...
| 输出格式 | 静态 WebP，质量 82，保留 alpha | 体积、清晰度和 WebView2 支持平衡较好 |
| EXIF | 只读取 Orientation 并应用到像素；输出不复制 EXIF | 避免双重旋转、隐私泄露和额外体积 |
| 资源并发 | Local 默认 2 worker，同时受 128MB 预计解码内存预算约束 | 防止两张超大图同时解码导致内存峰值 |
...
| 1 `ahead` | 向下 1.5 屏 | 正常向下滚动预生成 |
| 2 `behind` | 向上 0.5 屏 | 方便短距离回滚 |
| 3 `idle` | 停止滚动后额外向下 1 屏 | 仅 CPU 和队列空闲时运行 |
...
默认 WebP 质量为 82。漫画文字、线稿和高频壁纸不使用质量 70，以避免块状和边缘模糊。
...
若两个任务预计内存总和超过 128MB，第二个任务等待。25MP 图片解码约需 100MB，通常只能单任务执行。
...
fb_thumbnail_cache_limit_mb    默认 512
```
...
| WebP编码本身抢CPU | worker≤2、内存预算、快速滚动暂停低优先任务 |
