...
> **2026-08-15 实施**:方案 A 已落地(`HARD_MAX_PIXELS` 4MP → 8MP,spec §6.3 同步修订)。
> 边界澄清:本报告 §6 方案 A 行"5.76MP 落到灰区"仅在**宽度闸同时放行**时成立——
> 灰区要求 `宽 ≤ bucket×1.5`,对 1800px 宽竖屏照即 bucket ≥ 1536(colWidth ≥ ~1152,
> 1–2 列大卡片)。默认 4 列(bucket 512)下 1800 > 512×1.5,宽度闸独立命中,仍为
> Required。若目标是默认列宽下也让 4–8MP 竖屏照让位 worker,需另行评估放宽
> `HARD_WIDTH_RATIO`(方案 F,语义变化更大,未实施)。
