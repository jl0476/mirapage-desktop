...
## 当前状态（Phase 1-8 主体完成——Phase 3 已于 3.5.0 收口五格式；3.0.x / 3.1.x 持续打磨）
...
| 3 | 压缩包五格式 | ✅ ZIP/CBZ（含 ZipCrypto/AES 密码）+ RAR4/RAR5（unrar_sys）+ 7z（sevenz-rust）+ 会话密码 + 远程 ZIP 流式（2026-08-25 交付，详见 3.5.0 行与 README「压缩包支持与限制」） |
...
| 7 | SMB 协议层 | ✅ `v0.1.0-module3.3.0-smb` 真实现（`source/smb/` 模块：`source` 5 方法实装 / `connection` TTL 懒回收 + 凭据解析 + 重建重试 / `real_transport` share_connect + query + read_block + 错误映射 / `mock_transport` 测试桩 / `transport` TransportError 分类 / `path` UNC 拼接 + share 根契约）；旧 `smb_impl.rs` 已删；accounts CRUD + keyring 凭据 + `test_connection` 沿用 M1（module3.2.0）。**2026-08-26 实机打通**（用户 NAS 192.168.50.168）：连接/认证/浏览/share 根/提升根/重启恢复/全量+Range 读全链验证（五层实机修复链 + `unc_rel` share 首段剥离见 3.5.1 行与 `.superpowers/sdd/progress.md`）；阅读器经 SMB 翻页阅读等残余体验项随用随验 |
| 8 | WebDAV 协议层 | ✅ 真实现（`reqwest` + PROPFIND + Range GET）。**2026-08-26 群晖实机打通**：自签证书开关 + parse_propfind 真实形态三修复 + 传输层重试（见 3.5.1 行） |
...
| 3.1.1 | 书签与列表页打磨 | ✅ 书签添加/跳转入口补齐并统一页码语义；Bookmarks/History/Likes/Shortcuts 四列表页复用搜索与分页组件，快捷方式采用统一行样式；新增窗口大小、位置、最大化状态退出记忆（`tauri-plugin-window-state`）。**2026-08-29 补**：窗口恢复尺寸钳位（`window_bounds.rs`）——插件 restore_state 对 SIZE 无条件 set_size（POSITION 才有显示器守卫），副屏断连后大尺寸恢复可超屏；lib.rs setup 末尾一次性事后钳位（最大化/全屏不干预），+5 单测。 |
...
| 3.2.0 | M1 通用显示层（media:// 统一协议） | ✅ `v0.1.0-module3.2.0-media-display`（spec：`docs/superpowers/specs/2026-08-18-smb-remote-media-design.md` §3，plan：`docs/superpowers/plans/2026-08-18-media-display-m1.md`，9 轮审查 rev4-rev9）：`media://` 异步自定义协议（`register_asynchronous_uri_scheme_protocol`，图片字节不进 IPC；URL 单段 encode/decode 恰好一次 + 六形态固定段数 + 结构化路径校验；GET/HEAD/Range 200/206/416/403/404/502 + no-store）+ trait 新增 `stat()`（Local fs::metadata / WebDAV HEAD+Content-Length+Last-Modified / Archive ZIP 条目解压后 size；SMB M2）+ WebDAV Basic Auth（账户行 + keyring 密码）与 Range 强契约（206 或 200 恰等长，防 M3 分块污染）+ keyring 凭据（CredentialStore 抽象 + 新建失败回滚/type 不可变/删除先凭据后 DB+warning + `keyring = "3"` 启用 + Accounts.vue 密码框/type 锁定/残留 warning + 修 upsertAccount IPC key 误传 acct）+ `test_connection` 实装（webdav 真握手/smb 明确报错）+ 阅读器去 Local-only（parseSourceDescriptor 按 type 判别 + 四类源统一 mediaUrl + 类型拓宽 SourceDescriptor）+ History/Likes/快捷方式打开防御放开 + 浏览跳转 descriptor 化（`pendingOpenLocation {descriptor,relPath}` + `openDescriptorAt`，Likes 远程源「浏览」跳瀑布流可达）+ 本地 ZIP 全链路（双击 CBZ 进条目视图 `openArchive`/`exitArchive`/面包屑 ZIP 名点击退出/`up()` 顶层退出，archivePath 绝对路径）+ 瀑布流通用化（`masonryDescriptor` currentDescriptor 优先；remote 取源 actor：try_submit 非阻塞 + 并发 4/在途 64MB 双闸 + epoch 双检查 + PreparedRemoteTask 分类时刻完整快照）+ WebDAV 跨卷（`listing_kind` Local+WebDAV 放行 factory 列目录，SMB/Archive 明确报错）+ 远程图片预读预载（`media_cache.rs` 手写 LRU 256MB head=最新 + warm 会话协议 `advance_warm_session`/`warm_media_urls` 命令 + ReaderView openBook 后 await advance + 翻页预取后 3 张 + 卸载作废 + 账户变更整表清空）。**架构**：`Db` 内部改 `Arc<Mutex<Connection>>` + Clone（State<Db> 零改动，factory/WebDav 源持句柄取凭据）。**mediaUrl 直拼 URL**（convertFileSrc 对已编码 segment 二次 encodeURIComponent 双重编码，实测实锤后弃用）。前端 1096→1119（+23），Rust 337→371（+34），type-check 0 err，build 过。**手测清单 8 项待实机**（spec §3.7：本地三模式回归/本地 CBZ/远程 masonry/WebDAV 全能力矩阵/预读预载/Range 206·416/URL 攻击面/keyring 目检）。M2（SMB，module3.3.0，见 §Phase 7 行）/ M3（远程 Archive 物化，module3.4.0，见下行）均已落地。 |

| 3.4.0 | M3 远程 Archive 物化 + 预载 + cache | ✅ `v0.1.0-module3.4.0-remote-archive`（spec：`docs/superpowers/specs/2026-08-19-remote-archive-m3-design.md`，plan：`docs/superpowers/plans/2026-08-19-remote-archive-m3.md`，6 轮计划审查 rev2-rev6）：SMB/WebDAV 上的 CBZ/ZIP 整包下载至本地 `app_cache_dir()/archive-cache` 物化后按本地 ZIP 读（解压条目仍走 media://，字节不进 IPC）。**migration 016** `archive_cache` 索引表（cache_key PK + origin_kind/rel_path + origin size/mtime 快照 + LRU last_accessed_at；只存 ready 态，`.part` 不入表断点续传靠文件系统）+ settings `archive_cache_max_mb` 默认 2048。**Rust `source/archive/`**：`dao`（upsert/touch/evict LRU+protected/clear 返回路径列表实删文件/usage）/ `materializer`（`cache_key` sha256 `\0` 分隔 + `is_stale` 失效矩阵纯函数；`ensure_cached` 状态机——表命中→远端 stat 二次校验→失效**条件删除**（防并发 owner 完成交错删新文件）→`.part`+`.part.meta` sidecar 断点续传**四关严格校验**（cache_key 重算/canonical origin+rel/快照 size+mtime vs 远端/downloaded==.part 长度且≤远端 size，任一不符弃 `.part` 重头）→rename→upsert；网络错误保 `.part` 可续传、重试 1 次、in-flight 去重（Notify 持锁 `enable()` 预注册封丢唤醒窗口）、DB 连接不跨网络 await；**cancellation generation** 单调 AtomicU64 四检查点（chunk 前/二次 stat 前/rename 前/upsert 前）取代终值 epoch——防强制物化 rename/upsert「复活」已清缓存；`clearing` 闸门与 in-flight 注册同一把 async Mutex 单临界区（rev5 封 TOCTOU）+ `wait_inflight_drained` 2s 兜底）/ `prefetch`（三级：metadata stat 预热/内容低优 `ensure_cached_cancellable`/epoch 取消）。**接入**：`archive_impl.rs` 三方法接 Materialize trait——origin None 本地直开零回归/Some 物化；factory 断环构造 + `app.manage(factory.archive_materializer())`；`MaterializeError` 类型化 NotFound/Network 穿透保 media:// 404/502 语义；启动 `startup_cleanup`（孤儿 `.part`/孤儿 ready 文件/超容量 LRU；仅遍历 `.part` 数据文件防 `.part.part.meta` 误判删有效 sidecar，启动零网络请求）。**webdav**：PROPFIND 条目 `is_archive` 按扩展名判定（远程 ZIP 浏览入口）。**前端**：`openArchive` 泛化——远程源虚拟 archivePath（WebDAV=`{baseUrl}/{rel}` URL 形态 / SMB=`smb://{accountId}/{initialPath}/{rel}` 可读虚拟形态非真 UNC，虚拟路径零解析消费方仅展示与身份用）+ `archiveParent` descriptor 形态（exitArchive 恢复）+「正在准备压缩包」进度文案（`archive://progress` 事件）；`useArchiveWindowPrefetch` masonry 窗口预载接线（空窗口/换源/卸载必推 epoch 取消）；Settings remote section（预载开关/容量上限 512–32768 钳位/清空 confirm+用量展示）+ i18n 双语。**测试**：Rust lib 416→439（+23）+ 12 integration 全绿；前端 1126→1144（+18）；type-check 0 err。**tag 后用户复审修复轮**（commits 14a6aec/ab4b097/dde34a4，4 P1 + 3 P2）：`ensure_cached_cancellable` 改 3 参 `expected_epoch` 身份穿透（注册临界区内比较 + chunk 检查点比显式身份，删 `epoch_at_start` 捕获；`advance_epoch` fetch_max 单调；前端计数器播种 `Date.now()` 递增）封「检查-启动间隙竞态」；waiter 接管循环（≤5 轮——预载 owner 被 epoch 取消后强制 waiter 醒来重走「查表→抢 owner」续传，不再继承取消错误）；命中路径磁盘一致性校验（`cache_file_matches` 存在+长度==byte_size，失配条件删行重物化；启动反向扫表清悬空行）；`.part` 计入容量预算（`enforce_budget`：total=ready+parts，超限先淘汰最旧非 in-flight `.part` 到 80% 水位再 ready LRU；info 载荷加 partCount/partBytes，Settings 显示含未完成）；LRU 统一回收到 limit×0.8 水位（超限才动）；进度事件按当前 `archiveRelPath` 过滤（防他包串台）；mock 远端真 ZIP→物化→ArchiveMediaSource 解压→读 entry e2e（cbz+zip 双变体）。修复后 Rust 456 lib + 前端 1157 + type-check 0 err。**待实机手测**（本环境无 GUI/NAS，M1/M2 同款标注）：远程 8 项——WebDAV 首开准备态→条目视图→三模式阅读/二次秒开无网络请求/远端变更失效重下/断网+重启 `.part`+sidecar 续传（请求 offset≠0）/超容量 LRU 淘汰+手动清空/远程 ZIP masonry 缩略图/NAS SMB 复跑/预载开关关闭无窗口预取；本地 GUI 手测——Local ZIP 零回归/Settings 用量·上限·清空。 |

| 3.5.0 | RAR/7z 密码 + 远程 ZIP 流式读取（五格式收口） | ✅ 2026-08-25 交付，tag `v0.1.0-module3.5.0-archive-password-streaming`（含最终整分支双域审查 0 Critical + 2 Important 修复闭环——7z 预检非对齐密文归 CorruptArchive、候选期间页内导航推进 epoch 防迟到 ready 劫持；终态 Rust lib 579 + 前端 1190 全绿）（spec：`docs/superpowers/specs/2026-08-20-archive-rar-7z-password-streaming-design.md`，任务 1-14 全交付）。**五格式统一 Service**——`source/archive/` 新增 `backend.rs`（错误契约 + 资源上限 MAX_ENTRY_BYTES 512MiB / catalog 10 万 / 路径 4096B + LimitedEntryWriter）/ `password.rs`（会话密码库，Zeroizing 内存、验证后才缓存）/ `zip_backend.rs`（路径化 + ReaderFactory，无整包 Vec）/ `rar_backend.rs` + `rar_callback.rs`（unrar_sys 低层 API + data callback 硬上限，宽字符 encode/decode_wide 平台适配）/ `sevenz_backend.rs` + `sevenz_header_precheck.rs`（header 预检：BCJ×6/Delta/AES-CBC 受限解码 + dictionary/条目数上限）/ `service.rs`（catalog+password LRU、weighted 内存预算 semaphore、增长-回退协议）/ `cache_coordinator.rs`（清 cache 全局闸门）/ `remote_zip.rs`（远程 ZIP 尾部 Range 首开，失败降级完整物化）/ `commands/archive_access.rs`（prepare/unlock 可取消 IPC）。**密码语义**：仅会话内存——不进 descriptor/DB/日志/事件/sidecar/错误文本（泄漏守卫测试）；密码缓存清除后已缓存加密 catalog 不再返回 ready。**media:// 错误映射**（任务 14）：PasswordRequired/WrongPassword→423 LOCKED、EntryNotFound→404、ResourceLimitExceeded→413、Network→502、其余 Archive（含容器不可读归 Io，无 PermissionDenied 变体）→422，响应体固定短语零细节。**明确拒绝**：multi-disk ZIP / 分卷 RAR / 分卷 7z 专用错误；7z **BCJ2 不支持**（预检 UnsupportedCodec）；**损坏 start header 预检 CorruptArchive 不模糊恢复**（README「压缩包支持与限制」声明）。**依赖**：zip =2.4.2(+aes-crypto) / unrar_sys =0.5.8 / sevenz-rust =0.6.1(aes256+compress，上游 cfg 门禁缺陷妥协) / lzma-rust =0.1.7 / aes+cbc / zeroize =1.8.1（锁 zeroize_derive 1.4.2 防 edition2024——**勿裸跑 cargo update**）；24 个 fixture（WinRAR 7.11/6.24 + pyzipper + 手写构造性 7z + KAT 校验）入仓 `src-tauri/tests/fixtures/archive/`。**MSRV 1.75**：全树被预存在 smb→aead edition2024 阻断（任务 1 取证），`src-tauri/archive-msrv-smoke/` 隔离 smoke crate 正向证明（实际调用 zip/unrar/sevenz/password API + 锁文件钉版断言测试）；CI `verify.yml` windows job 末尾 `cargo +1.75.0 test` smoke manifest + 新增 ubuntu-latest `cargo check` job（守卫 rar_callback cfg(unix) 宽字符分支；macOS 随 Phase 9）。**BUILD.md** 补 Windows C++ Build Tools 为 UnRAR 硬要求 + THIRD_PARTY_LICENSES/UNRAR.txt 链接。**验证**（任务 14 全量）：Rust lib 578 passed 0 failed + integration 12；前端 1189 passed；type-check 0 err；`npm run build` + `cargo build` + `tauri build --no-bundle` 全过；七格式 fixture 过滤器（zip_backend 10 / rar_backend 11 / sevenz_backend 10）+ source::archive 160 + archive_access 11 + media_protocol 10 全绿。**待实机手测**：本环境无 GUI/NAS——spec §13.1/13.2 手工矩阵（本地五格式浏览/缩略图/阅读 + 密码重试/复用/重启重问；远程 WebDAV/SMB Range 首开/降级/续传/LRU）。 |
| 3.5.1 | 网络源实机修复轮（2026-08-26，用户 NAS 上 SMB + 群晖 WebDAV 全链打通；直推 main 未打 tag） | ✅ commits `325fcc4`→`23fb6e5`（7 个）。**A 选根目录支持网络源** — FileBrowser 选根按钮改 `PickRootMenu` 下拉（本地文件夹 / 网络账户列表打开即刷新 / 远程会话内「将当前目录设为根目录」）；store 新增 `canSetRootHere` + `setCurrentDirAsRoot`（SMB 空 initial 查 `listAccounts` 补 share 首段 / WebDAV 拼 path；提升改变身份拆分与本地换根语义一致）；**远程根持久化** `file_browser_last_remote_descriptor` 与 `LAST_ROOT_KEY` 互斥清写（currentDescriptor watch，Archive 视图不动键），启动恢复远程优先 + `hasActiveSource` 守卫修「远程会话重挂载被旧本地键拽回」；`saveNavigationContext` 远程/压缩包会话跳过（rootPath 是陈旧本地值）；`openDescriptorAt` 非 Local 分支补 `clearSelection`；displayRoot SMB 解析账户真实 host（`smb://192.168.x.x/Other1/wall`，账户已删回退 account-N）/ WebDAV 带 path 锚点段。**B SMB `unc_rel` 镜像段修复** — 契约 initialPath 首段 === share，但 unc_rel 把 initial 整段拼到 share 根后产生 `\host\share\share\...` （提升根实机暴露 Object Path Not Found；mock 用 share=media+initial=media 镜像同名目录恰好绕过 404 测）——改为剥离首段，非空 initial 的 share/子路径形态首次真正可用。**C WebDAV 自签证书** — migration 017 `account.accept_invalid_tls`（默认 0 严格校验），`WebDavMediaSource::build_client` 统一 `danger_accept_invalid_certs` 随账户行走，表单 WebDAV 态 checkbox；Accounts 表单 WebDAV 隐藏无消费的端口框（host=完整 base URL 协议+端口+路径一格）。**D WebDAV parse_propfind 真实形态三修复**（群晖实机暴露，fixture 未覆盖）— ① propstat `<status>` Text 经 current_field 跨标签泄漏覆盖 is_collection/size（End 事件清理 + resourcetype 独立 `in_resourcetype` 标志）② href 绝对路径 `/home/xxx/` 改 basename+percent-decode ③ Depth:1 自身条目按请求 URL path 过滤（签名 prefix→request_url）。**E test() HEAD→PROPFIND Depth:0**（群晖目录 HEAD 403）。**F 传输层抖动 `send_retry_once`** — 只读幂等方法（PROPFIND/GET/HEAD）失败换新连接重试一次（对齐 SMB connection 重连一次模式；list/read/test/stat 四点接入）。**G `.tb-btn` 提升全局**（styles/tailwind.css @layer utilities + `white-space: nowrap`）——scoped 定义对 PickRootMenu/Settings 等子组件不生效（选根按钮折行暴露；Settings 12 按钮一直裸样式）。**验证**：Rust 588 lib + 8 + 4 集成 / 前端 1216 / type-check 0 err；实机：SMB 提升根+重启恢复（12717 项目录）/ WebDAV 测试连接 true + 深层导航（含中文名目录 93 项）+ 原失败路径 8/8。**已知留后续**：~~py7zr header 加密 7z 兼容缺口~~（2026-08-29 已收口，见 3.5.6 行 C 项）；WebDAV 阅读器读图/Range/预读预载（M1 手测清单远程项）随用随验。 |
| 3.5.2 | 远程读取总闸门 + WebDAV 连接复用 | ✅ 两阶段 RemoteGate（并发 8 + 512MiB ×2 记账 + 30s acquire 超时）覆盖 webdav/smb read/stat（spec `docs/superpowers/specs/2026-08-26-remote-read-gate-design.md` 三轮审查 51a1b1f，plan `docs/superpowers/plans/2026-08-27-remote-read-gate.md` 9 任务 TDD）：webdav 共享 Client（bad_tls 两实例 OnceLock + connect_timeout 10s + pool idle 300s，per-request 超时保留 10/15/30s 语义）；单响应上限（GET 256MiB / PROPFIND 32MiB，CL 预检 + `read_body_capped` 流式累加双保险，初始容量 min(CL,1MiB) 不预分配）；SMB 60s 读超时摘槽（`invalidate` 公开化，超时走重连路径）；media path singleflight（`fetch_remote_to_cache` watch 值语义单飞）+ generation 守卫（`put_if_generation` 临界区比较插入，封死账户变更 clear 后旧内容写回）收敛 warm 与 media:// miss（warm rev8 写前检查取消，保留启动前检查）。**验证**：Rust lib 589→607（+18 测试）+ integration 12 全绿 / 前端零改动 1216 passed / type-check 0 err / cargo check 过。**计划内嵌代码执行修正**：reqwest 0.12 流式方法是 `bytes_stream`（chunk_stream 不存在）/ `content_length` 在 Response 上非 HeaderMap / accumulate 的 E 需 Display 约束 / RawStat `modified_unix_secs` 是 Option（计划写裸 i64）/ matches! 模式需 `ref m` 防部分移动 / mock 挂起改锁内读决策锁外执行（MutexGuard 显式 drop 后跨 await 仍被 generator Send 分析判死）/ watch Sender 注册表需 Arc 包装 / 容量断言按 spec §11.1 原意修正（计划 <64KiB 与 1MiB 起步实现矛盾）/ singleflight 三测试加模块级互斥锁（cargo 并行下 clear_all 丢弃他测在途 owner 结果的串扰）。**待实机**：WebDAV 阅读连接复用率（netstat 观察新建连接速率）、快速翻页并发 ≤8 内存平稳。 |
| 3.5.4 | 实机批：缩略图协作式取消热修 + 远程矩阵 + 性能实测 | ✅ 2026-08-28（直推 main 未打 tag，打前查占号）。**热修（目录切换旧队列不清）**：实测暴露 epoch 切换只 drain pending、2 个 in-flight 慢任务霸占 worker 数十秒拖死新目录首屏——`GenerationJob` 内嵌 `abort: Arc<AtomicBool>`，`new_epoch`/`cancel_all` 对 epoch 落后的 in-flight 置位，`generate_thumbnail` 在阶段边界检查返回 `ThumbnailError::Cancelled`（旧 epoch Completed→Stale 映射不变不写索引，前端零改动；最大浪费一个阶段）。**性能实测**（报告 `2026-08-08-masonry-thumbnail-performance.md` §4.1，CDP rAF 采样）：可见 DOM 18、4K 原图直载 0、滚动 max 33.3ms（基线 313ms，9.4×）、>100ms 帧 0（longtask 0）、热缓存满帧 max 17ms/p99 16.8ms——五格全达标；生成吞吐 debug 慢 10-50× 口径注记，冷缓存 ≤500ms 待 release 实测（另发现 restore 路径 masonry 请求链不发请求的预存在前端缺口，待专项排查）。**远程矩阵 PASS**：WebDAV 7z 物化 8.6s→二次 108ms（78×）、密码 7z 解锁 1.6s、SMB RAR5 明文 2.3s→二次 173ms、密码 RAR 解锁、连接复用实锤（5006/445 各单条 ESTABLISHED 长连接）。**CI linux-check 首轮** run#204 全绿销账；portable exe 启动验证 PASS（CDP DOM 级无白屏 + 远程根恢复正常）。**新发现处置**（2026-08-28 下午两修一观察）**：① restore 后 masonry 缩略图请求慢 → **已修 `05d872a`**——windows watch 缺 immediate，挂载时窗口已非空则首批请求被串行化在维度预取重算之后；immediate 后挂载即发（注册须在 flushRequest 定义后防 TDZ）。"请求链卡死"主控结论=三重测量假象叠加（debug 生成慢 10-50×+探针假 mtime 使 cache key 身份全错+超时脚本残留定时器互扰），终态 15/15 全渲染。③ 快速 open→exit→open 偶发 archiveOpening 卡死/假本地路径 → **已修 `8501a71`**——exitArchive 无条件 currentDescriptor=null：prepare 未 commit 阶段（archiveParent=null 提前 return）把进入前的远程源清掉，activeDescriptor() 回退陈旧本地 rootPath → 下一次 openArchive Local 分支拼假绝对路径（entryNotFound 实机复现 4/4，原 25s 挂死同根因）；修后仅当 currentDescriptor 确为 archive descriptor 才清，实机 4/4 极速轮次全过（111-239ms）。② debug build heap corruption 一次（0xc0000374，未复现）。**⑤（2026-08-28 深夜定性，替代原 WebView2 假说）清空缩略图缓存/大目录缩略图风暴后 IPC 全域悬挂**：症状链=clear 后所有 list_directory 后端 ok 但前端 invoke promise 永不 resolve（loading 永挂、全目录加载中）；裸 CDP 证明主线程空闲（非前端冻结）；invoke hook 发现唯一悬挂命令=list_directory，其它命令秒回；最终后端也开始不响应（窗口假死/偶发退出）。live dump 符号化（dbghelp+PDB，栈 34240/6840/33348 三线程）：tauri ipc respond_async_serialized 在 tokio worker poll 中经 park → mio::Poll::poll → Condvar::wait_until_internal 睡死，另有 registration_set::release 与 Condvar::notify_one_slow 同栈——mio/tokio 调度层唤醒丢失或注册表损坏，触发前提=clear_thumbnail_cache（cancel_all+删文件+清索引）与大目录缩略图风暴并发。**终版定性（同日深夜二段取证）**：主线程 dump 符号化锁定 thread 3916 卡在 tauri_runtime_wry::handle_user_message → softbuffer(win32 CPU 软件合成) Command::send——**RDP 远程会话无 GPU，WebView2 渲染回退 softbuffer，主线程被 CPU 光栅化占死，而 Tauri IPC 回包/事件必须经主线程消息泵派发** → 后端正常+前端 promise 全挂。复现环境=测试全程在 RDP 会话（Microsoft Remote Display Adapter）；本机 RTX 2060 SUPER 本地控制台会话不走 softbuffer，大概率不受影响（待本地复测确认；GameViewer 虚拟屏同理警惕）。堆 corruption/冻结/20s 自愈全部同源。**非产品缺陷、非 tauri bug，是 RDP 测试环境伪影**。处置：① spawn_blocking 两刀保留（clear/request 同步锁+删文件在 async worker 上本就是错误形态，正确加固，Rust 616 全绿）② 冷缓存指标改在本地控制台会话测 ③ 转储留 D:\compile\crashdumps 备查。**④（2026-08-28 傍晚新增，release 2/2 稳定复现）冷缓存目录进 masonry 渲染进程冻结**：清空缩略图缓存（或缓存为空）→ setRoot 冷目录 → masonry → 1-2s 内渲染进程硬冻结（日志静默、target 级 CDP 全超时、Debugger.pause 无 paused 事件 = 原生级阻塞非 JS 循环；进程存活、浏览器级端点正常）。debug build 同流程不冻结（spinner 排队正常）。与 ② heap corruption 疑似同源（均冷缩略图高频生成/清空场景 + 原生层）。取证路径：管理员配 WER LocalDumps（键 mirapage-desktop-local.exe）→ 复现冻结 → 关窗触发转储 → WinDbg 分析。**冷缓存首图指标被本 bug 阻塞**（冻结发生在出图前）。**2026-08-28 晚 dump 分析结论**：双转储（宿主 134MB + 渲染器 640MB，`D:\compile\crashdumps\`）已抓，渲染器 51 线程全阻塞在 ntdll 等待（43）或 win32u（8），栈回扫上层帧**全部是 msedge.dll 原生内部（worker 线程池/消息泵/oneauth），无任何应用层帧（无 V8/JS、无 WebView2 host IPC、无 Rust 侧符号）**——定性为 WebView2 运行时（msedge webview 版本）内部死锁/卡死，非应用代码缺陷。待办：① 报微软 Edge/WebView2（附双转储）② 试 WebView2 Runtime 更新/回滚 ③ 应用侧唯一可行缓解=冻结检测（CDP target 无响应超阈值自动重启 webview 进程）。② heap corruption（0xc0000374）同属原生层，同一转储渠道覆盖。**⑥（2026-08-28 深夜二次会话发现并当日修复，`0154ecc` 引入 → `e6b47b6` 修复）SchedulerHandle Clone+Drop 组合误杀缩略图调度器**：bug⑤ 热修给 `ThumbnailService` 加 `#[derive(Clone)]` 后，`request_thumbnails`/`clear_thumbnail_cache` 命令尾的临时 clone 析构触发 `SchedulerHandle::Drop` 发 `Shutdown`，调度器 actor 无声退出——实机症状：进 masonry 全部卡片永久卡「排队」（CDP 实测 72/72 generating、0 img、`spawn_completion channel closed` 风暴 72 条；此后所有新请求 reply 在 submit 点被当场丢弃、前端永等）。修法（plan `docs/superpowers/plans/2026-08-28-thumbnail-scheduler-clone-drop-shutdown.md`，SDD 三任务 + 双轮审查）：删 `impl Drop for SchedulerHandle` 与 `Command::Shutdown`（actor 自持 tx、recv 永不 None，生命周期=进程，与 `0154ecc` 之前行为等价）+ 回归测试 `clone_handle_drop_does_not_shutdown_actor` 锁定句柄可自由 Clone。验证：Rust lib 617（+1）+8+4 集成全绿；实机 CDP：切 masonry 15s cached 11/imgs 11 → 35s cached 28/imgs 21，本会话 0 channel closed、60 worker DONE，目录切出切入二次请求 3 次全健康。**归因提示：`0154ecc`（当天 17:22）之后的「瀑布流卡住」复现优先按此 bug 归因，勿与 ④⑤（当时误判为 RDP softbuffer/WebView2 原生层，2026-08-30 更正：实为应用层 async worker 阻塞，见下行更正）混淆。****2026-08-30 根因更正（用户拍板）**：④⑤ 两个定性均为误判——⑤ 的 IPC 全域悬挂与 ④ 的冷缓存渲染冻结实为**同一应用层问题：async worker 阻塞**。`clear_thumbnail_cache` / `request_thumbnails` 原在 async worker 线程同步执行 Db Mutex + 批量 SQL classify + 数百文件删除，大目录缩略图风暴时把 IO driver 推进线程耗尽，await 命令（list_directory 等）回包永不就绪——与 `0154ecc` 代码注释所述机制一致，spawn_blocking 两刀即真修复（此前 commit 标题挂的「RDP softbuffer 终版定性」作废）；`0154ecc` 引入的 Clone+Drop 误杀调度器回归由 `e6b47b6` 修复，也属同一修复链。「RDP 测试环境伪影」「WebView2 运行时内部死锁」「报微软/更新 Runtime」「冻结检测自动重启 webview」「RDP 环境规避」等后续动作全部作废；双转储的 msedge/softbuffer 栈面只是受害面非根因，转储留 `D:\compile\crashdumps\` 仅存档。修后实机（3.5.5/3.5.6 批）未再复现。 | 
| 3.5.5 | SMB share 根 mediaUrl 403 + load-error 兜底 + 缩略图任务统一出队 + epoch 单调（`749277a`，2026-08-28 深夜二次会话，直推 main 未打 tag） | ✅ 方案 `docs/superpowers/plans/2026-08-28-media-url-smb-and-thumbnail-dequeue.md`（三轮审查闭环 R1×4+R2×3+R3×1+附带×2，SDD 六任务+终审）。**A** `mediaUrl` smb/archive-smb 分支共用 `smbUrlParts` 换算（空 initialPath 取 share 首段、非空剥前缀）——share 根浏览（initialPath=""）下 `/smb/2//H%2F...` 空段被 `validate_rel_path` 拒绝 403，原图直显与 reader 读图全白（实机 72 卡取证：12 张 USE_ORIGINAL 小图全 403、13 张 CACHED 正常）；media handler 补 403/404/502 WARN 日志（error_to_status 映射表重构零漂移）。**B** `MasonryRow` 补 `@load-error` 接线（原 emit 悬空无人消费，img 失败纯白无提示）→ `markLoadFailed`（保留 cacheKey）→ `retryLoadFailed` 按来源分流（cached 先 `invalidate_thumbnail_cache_keys` 删文件+索引再 re-request——修"非空损坏 WebP 死循环"；original 直 re-request——修 resubmit 远程 unsupported）；popover retry 一并统一。**C** 出队统一（用户三拍板）：`currentPath` 纳入 epoch 失效键（切目录出队+清 stateMap，顺带修同名文件错图窗口）+ 卸载 bump + `cancel_keys` 双段取消（scheduler pending/inflight + RemoteFetchActor 按 (key,epoch) 绑定消费后删）+ `UseOriginal` 翻转时取消旧 GENERATE（service cache_key 顶部单算）+ `stale_on_completion` 防 cancelled 完成映射 Failed 反向覆盖 original 态（R3 P0）。**D** epoch 跨会话单调：模块级分配器 `max(Date.now(), last+1)` + `epochReady` notify 屏障（request 必在 notify 完成后，await 后 re-check）+ Rust scheduler 拒回退守卫 / fetch actor `fetch_max`（原无条件 store 可被乱序 IPC 拉回旧值）。**验证**：前端 1283 / Rust lib 627（+8）/ type-check 0；实机 CDP：epoch 毫秒级单调、切目录/卸载/重挂三路径出队（inflightAborted=1 + err=cancelled）、注入 404→failed 卡+↻→点击→generating→恢复、invalidate 命令实机可用、全程无空段 URL、运行时零 regressed（日志中 20 条 regressed 全为 cargo test 触发——测试写生产 main.log 属预存在现象）。**新观察（预存在，未修）**：measuredMap 维度污染、navigate() 失败不回滚——两项已于 2026-08-29 3.5.6 小件收口（见该行）。 |
| 3.5.6 | 遗留收口小件（§16.2.1 四项 + 窗口钳位 + py7zr header 加密） | ✅ 2026-08-29（直推 main 未打 tag）。**A §16.2.1 四项**（commit `c68cfe2`）——① measuredMap 维度污染：classify 输入与布局比例分双 map（`sourceDimsMap` 只收 header 预读真值，缩略图 onLoad natural 只喂布局；popover 源图尺寸同改）；② navigate 失败回滚（fetch 返回 ok/stale/error 三态，error 回滚 currentPath+searchQuery，stale 归最新请求裁决）；③ retryLoadFailed/retry/regenerate IPC reject 置回 failed（不再永久 spinner）；④ MasonryView 三处接线补组件级 spy 断言。前端 1283→1294。**B 窗口恢复尺寸钳位**（commit `f44de24`）——window-state 插件 restore_state 对 SIZE 无条件 set_size（POSITION 才有显示器守卫），副屏断连后大尺寸恢复可超屏；`window_bounds.rs` 纯函数 + lib.rs setup 末尾一次性事后钳位（最大化/全屏不干预），+5 单测（Rust 627→632）。**C py7zr header 加密收口**——`f90078a` 已放宽 MAX_HEADER_DICT_BYTES 8→32 MiB（py7zr header 压缩流固定 16 MiB dict），本批实测定位剩余缺口：py7zr set_encrypted_header(True) 产物 header **不解压缩**（单 AES256SHA256 coder），声明 unpack 189 / 块填充密文 192，受限 decoder 原把超出声明值判 CorruptArchive；改按 7z 语义以声明 unpack size 截断块填充（上游 sevenz-rust/7-Zip 同语义，真伪由 folder CRC 兜底，危险分配防线不变）。收编 py7zr 1.1.3 产物 fixture ×2（py7zr-plain.7z / py7zr-header-encrypted.7z，gen_py7zr.py 生成，fixture 清单 24→26；header 加密变体随机 salt/IV 构造上不可复现，按 README 内容锁定规范以已提交产物为真值）+ 2 回归测试（16 MiB dict 合法对照 / 无密码 PasswordRequired + 正确密码截断后可读），+1 临时探针测试（py7zr 内容加密 header 可见 probe 回 first_encrypted_image 契约锁定）。Rust lib 632→634。**附带**：.gitignore 加 `.mimosa/`。 |
| 3.5.3 | 小件打包六修复 + 挂账销账 | ✅ commits `12f7ef5`→`d710d7a`（7 个；spec `docs/superpowers/specs/2026-08-27-module3.5.3-small-fixes-design.md` 四轮审查闭环，plan `docs/superpowers/plans/2026-08-27-module3.5.3-small-fixes.md`）。**A 瀑布流陈旧尺寸守卫** — 切目录重置 measuredMap + 预读回包 identity 快照比对（descriptorId 与 path 拼接的稳定语义键，本地源字面量 descriptor 引用不稳不误判）；**B archiveOpenError 随导航/退出清除** — 修跨目录持久横幅残留；**C Settings remote 开关/缓存上限回滚** — invoke 失败回滚 UI 假成功 + alert 直呈错误；**D RAR ERAR_EOPEN 归类 CorruptArchive→Io** — 容器打不开≠损坏，整表特征锁防漂移；**E migration 018 清理孤儿设置键 archive_cache_size_mb** — 版本断言 17→18；**F ready fresh 校验复用 identity stat** — 已物化 GET 远端往返 4→2（F3 砍半版）；任务 7 注释收口（四处防误开标注：三 backend catalog + buildArchiveCandidate）。**挂账销账**：loadingText en 全角括号不成立 / 嵌套包双击由构造消除 / canonical 两表单侧自洽不做迁移；**留后续池见 DESIGN §16**。**验证**：Rust lib 607→612（+5）+ migrations 30 + integration 12 全绿 / 前端 1243→1257（+14）/ type-check 0 err / build 过。 |
| 3.5.7 | 瀑布流测量批次 scrollTop 锚定补偿（§16.2 G 项兑现） | ✅ 2026-09-04 直推 main（plan `docs/superpowers/plans/2026-09-03-masonry-measured-anchor.md` 五轮审查 17 项修入后内联执行）。**根因**：3:4 估算占位被竖屏截图真实比（856×1920，+68%）替换时视口上方内容下推 300-526px（CDP 探针 4s/8 跳、体感合并成 2 次）；孤儿补偿函数 applyMeasuredBatch 自 3.0.6 E1 简化后零调用方，MasonryView.vue 注释自认挂账 G 项。**修法**：新增 commitMeasuredMap 单一提交口收编 onRowMeasured/triggerDimensionPrefetch 两路径——批内首次提交按批前 layout 捕锚（captureMasonryViewportAnchorLoose：严格命中优先，顶线落所有列 gap 时 fallback 下边缘最大上方图 + belowOffset 绝对偏移，restore 分流互斥字段、resize 严格路径字节级不变），measuredMap 写入**先于** nextTick 注册（Vue 无 pending flush 时先注册会让恢复跑在 flush 前、守卫空转）。**竞态防护**：批次 anchor/key/seq 闭包化（回调先验 seq 再动共享态，旧批不清新批）；目录切换 watcher 同步失效测量 + resize 两套锚（seq 越过 + resizeAnchor/resizeEndTimer 清理）；resize↔测量双向让位（resizeAnchor 活跃期间测量复用 resize 锚重触发恢复；RO 开始即失效 pending 测量批）——单一恢复源双向闭环。**测试**：前端 1294→1306（+16 新 −4 删孤儿用例）；useMasonryLayout mock 升混合派生（fakeLayoutMap 非空走静态预填、为空从 measuredMap 派生——header 异步批用例的手动翻布局在微任务泵下进不了 capture/restore 窗口）；顺手修 3 处预存在 mount-不-unmount 泄漏（Settings×2/布局输入用例，prefetchMock 响应化后泄漏实例会跨用例消费 IPC）。**待实机**：E2E 探针 v4（用户输入判静止 + 固定初始基线累计漂移 maxAbsDrift≤8 + anchorLost=0）复测同目录滚动。 | 
...
- **网络账户表单语义**（2026-08-26 实机定稿）：WebDAV「主机」= **完整 base URL**（协议+端口+路径一格写全，如 `https://ip:5006/home`），端口框仅 SMB 消费（WebDAV 态隐藏）；SMB share 名写裸名（前后斜杠写侧归一）。自签证书豁免是 **per-account** 开关（`accept_invalid_tls`，migration 017，默认严格校验）——不是全局设置。
- **SMB initialPath 契约**（2026-08-26 实机定稿）：空串 = share 根；非空首段必须 === account.share（形如 `share/sub/dir`），`unc_rel` 拼 transport rel 时**剥离首段**（tree 已在 share 内，拼入即 `\\host\share\share\...` 镜像重复段）。提升当前目录为根改变 (descriptor, relPath) 身份拆分——与本地换根语义一致，同目录经不同根浏览是两个身份。
- **远程根持久化**（2026-08-26）：`file_browser_last_remote_descriptor`（SMB/WebDAV descriptor JSON）与 `file_browser_last_root`（本地路径）互斥清写、启动恢复远程优先；Archive 条目视图不动两个键（重启回到压缩包所在目录的根）。
