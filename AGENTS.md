...
| 3.5.3 | 小件打包六修复 + 挂账销账 | ✅ commits `12f7ef5`→`d710d7a`（7 个；spec `docs/superpowers/specs/2026-08-27-module3.5.3-small-fixes-design.md` 四轮审查闭环，plan `docs/superpowers/plans/2026-08-27-module3.5.3-small-fixes.md`）。**A 瀑布流陈旧尺寸守卫** — 切目录重置 measuredMap + 预读回包 identity 快照比对（descriptorId 与 path 拼接的稳定语义键，本地源字面量 descriptor 引用不稳不误判）；**B archiveOpenError 随导航/退出清除** — 修跨目录持久横幅残留；**C Settings remote 开关/缓存上限回滚** — invoke 失败回滚 UI 假成功 + alert 直呈错误；**D RAR ERAR_EOPEN 归类 CorruptArchive→Io** — 容器打不开≠损坏，整表特征锁防漂移；**E migration 018 清理孤儿设置键 archive_cache_size_mb** — 版本断言 17→18；**F ready fresh 校验复用 identity stat** — 已物化 GET 远端往返 4→2（F3 砍半版）；任务 7 注释收口（单层压缩包约束 + buildArchiveCandidate 可达性边界标注）。**挂账销账**：loadingText en 全角括号不成立 / 嵌套包双击由构造消除 / canonical 两表单侧自洽不做迁移；**留后续池见 DESIGN §16**。**验证**：Rust lib 607→612（+5）+ migrations 30 + integration 12 全绿 / 前端 1243→1257（+14）/ type-check 0 err / build 过。 |
