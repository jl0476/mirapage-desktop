...
| 3.0.7 | Library → Likes 合并 | ✅ `v0.1.0-module3.0.7-likes-merge`（spec：`docs/superpowers/specs/2026-08-13-library-to-likes-merge-design.md`，plan：`docs/superpowers/plans/2026-08-13-library-to-likes-merge.md`）：去掉冗余"书库"+ 死表 `like`，统一为"喜欢"单一概念。**数据层**：migration 011 `UPDATE library SET is_favorite=1 WHERE id IN (SELECT book_id FROM \`like\`)` 先合并数据再 DROP `like` 表（不丢用户数据；`like.book_id` 必有对应 `library.id`，UPDATE 安全）。**视图层**：删 `Library.vue` + `/library` 路由改为 `redirect: '/likes'`（兼容 dev hot reload / 调试 / 未来 deep-link）；重写 `Likes.vue` 用 `useLibraryStore.favorites` + 行内 ❤️ toggle（图标 ⭐→❤️，复用 `library.toggleFavorite`）；`router/index.test.ts` 新增（`push+isReady+fullPath` 验证 redirect，不用 `resolve()`——round 3 P1 反馈指出不可靠）。**Reader**：删主菜单"加入书库"按钮（跟 ❤️ toggle 数据语义重叠，合并为单一入口）；新增 `onToggleLike` 本地 handler 调 `setFavorite(id, !isFavorite)` + **同步 `book.value.isFavorite`**（修两个 bug：① 原本 toggleLike 写 like 表跟 :is-liked 读 library 数据源不一致；② book 是 get_book 快照不刷导致同会话无法取消喜欢）；`ReaderView.test.ts` 新增连续 toggle 测试。**SideNav**：删 library 项，likes 提到第 3 位（原 library 位置）；`SideNav.test.ts` 8→7 link。**FileBrowser 三入口**：`FileBrowser.vue` / `EntryDetailPanel.vue` / `RowContextMenu.vue` 文案 `fileBrowser.addToLibrary` → `reader.like`（喜欢），避免认知割裂。**i18n**：删 `nav.library` / `library.*` namespace / `reader.menu.library` / `fileBrowser.{,contextMenu.}addToLibrary` / 孤儿 `fileBrowser.inLibrary`。**3 轮代码审查 9 条反馈全闭环**：R1（migration 011 非 009 + 数据合并 + commands/mod.rs 删 + SideNav.test 改 + Reader book ref 同步 + Likes reader params）；R2（ReaderView.test 加连续 toggle + /library redirect）；R3（redirect 测试用 push 不用 resolve）。**有意差异 vs Android**：保留桌面端 redirect 兜底（Android LibraryEntity 名字不变，但桌面视图重命名"喜欢"语义更明确）。**预存在不修**（留后续）：RowContextMenu.onResetProgress 查 book_id 落空（list_library WHERE is_favorite=1 不含 temp-imported）；FileBrowser 入口按钮无"已喜欢"状态联动。单测 582→718（注：实际 baseline 比 spec 估的 580~586 高，spec 写时 baseline 错估）。 |
