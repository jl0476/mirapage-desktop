...
- 单测跑：220 → 259 → 393 → 397 → 462 → 535 → 582 → **717** 用例（v0.1.0-module3.0.8 缩略图 polish + 浏览位置 + fix 集群增量：MasonryView 17 + MasonryThumbnail 4 + MasonryThumbnail.integration 1 + ThumbnailCacheSettings 4 + useMasonryThumbnails 13 + useMasonryBrowsePosition 19 + useReaderActions + reader store + Settings store 共 ~50 + 各项回归），目标 0 fail。Rust lib 单测 179 中 **177 pass / 2 fail**（详见 `docs/superpowers/reports/2026-08-11-feature-matrix.md` §1.5）。
