...
| 3.0.4 | 文件浏览器虚拟列表 | ✅ `v0.1.0-module3.0.4-virtuallist`（spec：`docs/superpowers/specs/2026-08-06-large-folder-perf-design.md`）：手写 `useVirtualList` composable（~80 行）+ FileList 三视图统一虚拟化 + viewMode 切换 DOM 复用（CSS `:not()` 显隐）+ 算法层顺手修 4 个 O(n²) hot path（`pathIndex` O(1) / `toggleSelection` in-place + `triggerRef` / `readStatus.finishedSet` O(1) / `displayedEntries` 单次循环合并 fast path）。**E2E 实测**：14949 entry "AI" 目录 DOM 节点 194,485 → 1,284（**151×**）；JS heap 167 MB → 32 MB（**5.3×**）；搜索 "page" 后 DOM 1,284 → 137（**1,415×**）；hover 200 次 avg 0.002ms / longtask 0；滚动到底 clamp 正确。msedgewebview2 总内存预期 1.5 GB → 300-500 MB。完整 E2E 报告 `docs/superpowers/reports/2026-08-06-virtuallist-e2e.md`（commit b6ad078）。 |
